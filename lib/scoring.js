import { nomeLivello } from './livelli';
import { DISTRETTI } from './distretti';
...
export function aggregateNMQ(allAnswers) {
  ...
  if (n === 0) {
    return {
      zones: [], districts: [], level1: empty, level2: empty, level3: empty,
      prevalence: empty,
  ...
  // Distretti corporei (lib/distretti): servono quando la popolazione è piccola e
  // le singole zone finiscono tutte sotto la soglia di riservatezza. Una persona
  // conta una volta sola per distretto, anche con più zone colpite.
  const districts = DISTRETTI.map(d => {
    let c = 0;
    for (const a of allAnswers) {
      const s = scoreNMQ(a);
      if (d.zone.some(zi => s[zi] && s[zi].q12)) c++;
    }
    return { key: d.key, zone: d.nome, dettaglio: d.dettaglio, count12: c, pct12: Math.round(c / n * 100) };
  }).sort((a, b) => b.pct12 - a.pct12);
...
export function generateSummaryText(nmq) {
  ...
  if (nmqColor === 'red') {
    return `Il ${l1pct}% dei dipendenti presenta disturbi muscolo-scheletrici con impatto funzionale (Livello 1) e necessita di trattamento osteopatico. Un ulteriore ${l2pct}% è in Livello 2 (dolore senza impatto, ${nomeLivello('level2').toLowerCase()} attiva). La zona più critica è ${zoneTxt}.`;
  }
  if (nmqColor === 'yellow') {
    return `Il ${l1pct}% dei dipendenti è in Livello 1 (disturbi con impatto funzionale, candidati al trattamento), il ${l2pct}% in Livello 2 (${nomeLivello('level2').toLowerCase()} attiva). La zona più colpita è ${zoneTxt}.`;
  }
  return `Il ${l1pct}% dei dipendenti è in Livello 1 (${nomeLivello('level1').toLowerCase()}) e il ${l2pct}% in Livello 2 (${nomeLivello('level2').toLowerCase()}). La popolazione mostra un quadro muscolo-scheletrico complessivamente buono; zona prioritaria: ${zoneTxt}.`;
}
...
export const TYPE_LABELS = {
  initial: 'Check-up iniziale',
  '3month': 'Checkpoint 3 mesi',
  '6month': 'Review 6 mesi',
  final: 'Check-up finale',
};
