...
import { VOCI_PROGRAMMA, RIGA_CHIUSURA } from './programma';
import { legendaHtml } from './livelli';
...
export function buildQuoteHtml({ client, forchetta, sector_label, v2Doc, v2Params }) {
  const now = dataIt(new Date(), { year: 'numeric', month: 'long', day: 'numeric' });
  const f = forchetta || {};
  ...
  // Sezione componenti (SOLO listino v2): le 12 voci di Enrico (fonte unica
  // lib/programma.js), senza tariffe né valori. L'ergonomia di produzione resta una
  // voce STIMATA "da confermare con il sopralluogo".
  const escHtml = t => String(t).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const v2Section = v2Doc ? `
    <h2>Cosa comprende il programma</h2>
    <div class="voci">
      ${VOCI_PROGRAMMA.map(v => `<div class="voce"><div class="vt"><span class="vn">${v.n}</span>${escHtml(v.nome)}</div><p>${escHtml(v.cliente)}${v.nota ? ` <em>${escHtml(v.nota)}</em>` : ''}${v.n === 7 && v2Doc.ergonomiaPostazioni > 0 ? ' <em>La parte relativa alle postazioni di produzione è stimata e verrà confermata con il sopralluogo.</em>' : ''}</p></div>`).join('\n      ')}
    </div>
    <p class="chiusura">${escHtml(RIGA_CHIUSURA)}</p>
    ${v2Doc.quantita && v2Doc.quantita.length ? `<div class="quant"><div class="qt">Nel vostro programma, primo anno</div><ul>${v2Doc.quantita.map(q => `<li>${escHtml(q)}</li>`).join('')}</ul></div>` : ''}` : '';
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .voci { display: grid; grid-template-columns: 1fr 1fr; gap: 12px 22px; }
  .voce { border-top: 1px solid #e2e8f0; padding-top: 8px; break-inside: avoid; page-break-inside: avoid; }
  .voce .vt { font-weight: 800; font-size: 12.5px; color: #2C3E50; margin-bottom: 3px; }
  .voce .vn { display: inline-block; min-width: 20px; color: #16a34a; }
  .voce p { font-size: 11.5px; color: #475569; line-height: 1.45; }
  .chiusura { margin-top: 14px; font-size: 12px; color: #2C3E50; font-weight: 600; }
  .quant { margin-top: 12px; background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 10px; padding: 12px 16px; }
  .quant .qt { font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; color: #16a34a; margin-bottom: 6px; }
  .quant ul { margin-left: 16px; font-size: 12px; } .quant li { margin-bottom: 2px; }
  ...
  @media print { body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .card, .step, .why, .clause { break-inside: avoid; } h2 { break-after: avoid; } }
</style>
</head>
<body>
<div class="page">
  ...
  <div class="body">
    ...
    <div class="iva" style="font-size:11px;color:#64748b;margin-top:6px">${DICITURA_IVA}</div>
    ${legendaHtml(v2Params)}

    ${v2Section}
    <h2>Perché una forbice e non un prezzo fisso</h2>
    <div class="why">
      <p>Il prezzo definitivo del programma dipende dalla <strong>composizione reale della vostra popolazione</strong> — quante persone presentano un effettivo bisogno clinico — ed è un dato che si conosce con certezza <strong>solo dopo il check-up</strong>.</p>
    ...
    <div class="steps">
      <div class="step now">
        <div class="num">1</div>
        <div><div class="st">Colloquio e Stima di investimento</div><div class="sd">Questo momento. Vi presentiamo il modello e la Stima di investimento — la forbice qui sopra — calibrata sulla vostra dimensione e sul vostro settore.</div></div>
      </div>
      <div class="step">
        <div class="num">2</div>
        <div><div class="st">Check-up della popolazione</div><div class="sd">Ogni dipendente compila un questionario digitale validato: la fotografia oggettiva dello stato di salute muscolo-scheletrica, in forma aggregata e riservata.</div></div>
      </div>
      <div class="step">
        <div class="num">3</div>
...
// ─── Stima PACCHETTO PREVENZIONE (template dedicato, cliente-facing) ──────────
// Naming parametrico (mai i nomi interni); NESSUN riferimento a trattamenti come
// inclusi; ergonomia produzione = voce stimata "da confermare con sopralluogo";
// prezzo unico (le componenti si elencano senza tariffe unitarie).
export function buildPacchettoHtml({ client, pacchetto, naming, sector_label }) {
  const now = dataIt(new Date(), { year: 'numeric', month: 'long', day: 'numeric' });
  const p = pacchetto || {};
  const eur = (x) => `€${Math.round(Number(x) || 0).toLocaleString('it-IT', { useGrouping: 'always' })}`;
  const c = client || {};
  const nome = naming || 'Pacchetto Prevenzione';
  // Conteggi numerici dal motore (prima si leggeva il testo di `detail`: fragile).
  const conPostazioni = !!(p.ergonomia && p.ergonomia.nPostazioni > 0);
  const conAddetti = !!(p.ergonomia && p.ergonomia.nAddetti > 0);
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .meta .v { font-size: 15px; font-weight: 700; color: #2C3E50; }
  h2 { font-size: 13px; text-transform: uppercase; letter-spacing: 1.2px; color: #16a34a; margin: 28px 0 14px; padding-bottom: 7px; border-bottom: 2px solid #16a34a; }
  .price { border: 2px solid #16a34a; background: #f0fdf4; border-radius: 12px; padding: 26px; text-align: center; }
  .price .eur { font-size: 34px; font-weight: 800; color: #16a34a; line-height: 1.05; }
  .price .yr { font-size: 12px; color: #64748b; margin-top: 4px; }
  .why { background: #f8fafc; border-left: 4px solid #16a34a; border-radius: 0 10px 10px 0; padding: 18px 22px; }
  .why p { margin-bottom: 9px; } .why p:last-child { margin-bottom: 0; }
  .clause { background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 12px; padding: 18px 22px; }
  .clause .ttl { font-weight: 800; color: #16a34a; margin-bottom: 6px; font-size: 14px; }
  .footer { background: #f8fafc; border-top: 1px solid #e2e8f0; padding: 18px 40px; font-size: 10px; color: #94a3b8; display: flex; justify-content: space-between; margin-top: 30px; }
  @media print { body { -webkit-print-color-adjust: exact; print-color-adjust: exact; } .why, .clause, .price { break-inside: avoid; } h2 { break-after: avoid; } }
</style>
</head>
<body>
<div class="page">
  ...
  <div class="body">
    <h1>Stima di investimento — ${nome}</h1>
    <p class="subtitle">Fotografia della salute muscolo-scheletrica, formazione ed ergonomia — 12 mesi</p>
    ...
    <h2>Investimento</h2>
    <div class="price">
      <div class="eur">${eur(p.price)}</div>
      <div class="yr">12 mesi${conPostazioni ? ' · quota ergonomia produzione stimata, da confermare con sopralluogo' : ''}</div>
      <div class="yr">${DICITURA_IVA}</div>
    </div>

    <h2>Cosa comprende</h2>
    <div class="why">
      <p>• <strong>Check-up completo della popolazione</strong>: ogni dipendente compila un questionario digitale validato, con gli stessi consensi e le stesse garanzie di riservatezza del programma completo. L'azienda riceve la fotografia aggregata e riservata dello stato di salute muscolo-scheletrica.</p>
      <p>• <strong>Formazione collettiva</strong> su postura e prevenzione (${inLettere(PROTOCOLLO.formazione_moduli_primo_anno)} moduli, gruppi in sede).</p>
      <p>• <strong>Consulenza ergonomico-posturale</strong> sulle postazioni di lavoro${conAddetti ? ', con la formazione degli addetti di reparto alla postura corretta sulla propria postazione' : ''}${conPostazioni ? ' — <em>la parte relativa alle postazioni di produzione è stimata e verrà confermata con il sopralluogo</em>' : ''}.</p>
    </div>

    <h2>Durata e prospettive</h2>
    <div class="clause">
      <div class="ttl">12 mesi, senza rinnovo automatico.</div>
      <p>Al termine dei 12 mesi il percorso si conclude oppure evolve, se lo desiderate, in un programma più ampio costruito sui dati emersi dal check-up. Nessun impegno oltre la durata indicata.</p>
    </div>
  </div>
...
<div class="cover">
  ...
  <div class="cover-body">
    ...
    <div class="cover-meta">
      ...
      <div class="item"><div class="label">Generato da</div><div class="value">Piattaforma digitale ES Work</div></div>
    </div>
    <div class="confidential">🔒 Documento riservato — uso esclusivo cliente</div>
  </div>
  <div class="cover-footer">
    <span>Essentia Salutis · info@essentiasalutis.it</span>
    ${report_type === 'activation' ? `<span>${DICITURA_IVA}</span>` : ''}
  </div>
</div>
