...
// I valori dichiarati del Listino (servizi_deliverable) sono solo per Enrico: MAI nei
// documenti del cliente (decisione 11/9) — lì le quantità del primo anno e l'investimento.
// ─────────────────────────────────────────────────────────────────────────────

// I numeri delle voci (sedute, durata, cicli) vengono dal protocollo, mai scritti a mano.
import { PROTOCOLLO, inLettere, fraseTemiFormazione, fraseTempoRichiesto } from './protocollo.mjs';
import { TESTO_OT23_IN_VERIFICA } from './ot23-stato.mjs';
...
export const VOCI_PROGRAMMA = [
  ...
  },
  {
    ...
    cliente: `Giornate di sportello in azienda secondo calendario concordato, in uno spazio riservato messo a disposizione dall'azienda. Sessioni individuali da ${PROTOCOLLO.durata_seduta_min} minuti con professionisti sanitari del network.`,
  ...
  },
  {
    ...
    cliente: `Per chi presenta dolore con impatto funzionale: ciclo di ${PROTOCOLLO.sedute_per_ciclo} sedute con rilevazione del dolore prima e dopo ogni seduta, fino a ${inLettere(PROTOCOLLO.cicli_trattamento_per_anno)} cicli per anno di programma per persona, con un intervallo minimo tra i cicli.`,
    argomentario: `${inLettere(PROTOCOLLO.sedute_per_ciclo, { maiuscola: true })} sedute non sono un compromesso al ribasso: sono il formato che permette di misurare un cambiamento e di documentarlo. Se chiedono "e se non basta": c'è il secondo ciclo, e comunque il programma non promette guarigioni — promette presa in carico e misura.`,
  },
  {
    n: 5,
    nome: 'Prevenzione individuale (Livello 2)',
    cliente: `Per chi presenta segnali senza impatto funzionale: ${PROTOCOLLO.sessioni_prevenzione_l2} sessioni di prevenzione per anno di programma per persona, dal primo anno di programma.`,
  ...
  },
  {
    ...
    cliente: `Sessioni collettive aperte a tutti i dipendenti, non solo a chi ha sintomi: ${fraseTemiFormazione()}, teoria e pratica sul gesto, in gruppi dimensionati sulla popolazione. Chi entra in azienda durante l'anno recupera la formazione con sessioni dedicate.`,
  ...
  {
    n: 12,
    // Testo di Enrico (21/9). Fino alla risposta scritta dell'INAIL sull'ammissibilità
    // del programma come intervento C-4.1, nessun documento promette l'OT23: niente
    // percentuali né cifre di riduzione collegate al programma (lib/ot23-stato.mjs).
    nome: 'Documentazione degli interventi erogati',
    cliente: TESTO_OT23_IN_VERIFICA,
    argomentario: `Fino alla risposta scritta dell'INAIL NON presentarla come una riduzione ottenibile: niente percentuali né cifre. Si può dire che il programma documenta pianificazione, presenze e risultati, che l'ammissibilità è in verifica e che la riduzione OT23 richiede comunque più di un intervento; la domanda la presenta l'azienda.`,
  },
];

export const RIGA_CHIUSURA = `Tutte le componenti sono comprese nel programma annuale e dimensionate sui dati della vostra popolazione. Il tempo richiesto è di ${fraseTempoRichiesto()}.`;

const eur = (v) => `€${Math.round(Number(v) || 0).toLocaleString('it-IT', { useGrouping: 'always' })}`;
const plur = (n, uno, molti) => `${n} ${n === 1 ? uno : molti}`;

// Quantità del programma nel primo anno, dal calcolo del prezzo (listino v2). Decisione
// Enrico: nei documenti del cliente MAI euro accanto alle voci — le quantità, e un solo
// numero in fondo (l'investimento). Mai il numero di persone del Livello 1: si dicono i
// cicli previsti, e nemmeno quelli se il gruppo è sotto la soglia di riservatezza.
export function quantitaPrimoAnno(calc, { mostraCicli = true } = {}) {
  if (!calc || !calc.y1) return [];
  const out = [];
  if (calc.days_osteo_y1 > 0) out.push(`${plur(calc.days_osteo_y1, 'giornata', 'giornate')} di sportello in sede`);
  if (calc.l1 > 0) out.push(mostraCicli ? `cicli clinici per i casi individuati dal check-up: fino a ${plur(calc.l1, 'ciclo', 'cicli')}` : 'cicli clinici per i casi individuati dal check-up');
  const prev = calc.y1.prevention;
  if (calc.l2 > 0 && prev && prev.sessions > 0) out.push(`${Math.round(prev.sessions / calc.l2)} sessioni di prevenzione a persona per chi presenta segnali senza impatto`);
  if (calc.training_sessions_y1 > 0) out.push(`${plur(calc.training_sessions_y1, 'sessione', 'sessioni')} di formazione per tutti`);
  const ergo = quantitaErgonomia(calc.y1.ergonomia);
  if (ergo) out.push(ergo);
  out.push('4 report nell\'anno (Attivazione, due review, annuale)');
  return out;
}

function quantitaErgonomia(e) {
  if (!e || !(e.sell > 0)) return null;
  const pezzi = [
    e.nUfficio ? `${plur(e.nUfficio, 'persona', 'persone')} in ufficio` : null,
    e.nPostazioni ? `${plur(e.nPostazioni, 'postazione tipo', 'postazioni tipo')} in produzione` : null,
    e.nAddetti ? `${plur(e.nAddetti, 'addetto', 'addetti')} di reparto` : null,
  ].filter(Boolean);
  return pezzi.length ? `consulenza ergonomico-posturale: ${pezzi.join(', ')}` : null;
}

// Quantità nella Stima (prima del check-up): lo sportello dipende da quanti casi emergeranno
// → forchetta di giornate tra lo scenario minimo e il massimo (decisione Enrico: "la forchetta
// sembra un metodo"). Formazione, ergonomia e report sono certi.
export function quantitaStima(forchetta) {
  if (!forchetta || !forchetta.min || !forchetta.max) return [];
  const out = [];
  const a = forchetta.min.days_osteo_y1, b = forchetta.max.days_osteo_y1;
  if (a > 0 || b > 0) out.push(a === b ? `circa ${plur(a, 'giornata', 'giornate')} di sportello in sede, dimensionate sul check-up` : `tra ${a} e ${b} giornate di sportello in sede, dimensionate sul check-up`);
  const base = forchetta.avg || forchetta.min;
  if (base.training_sessions_y1 > 0) out.push(`${plur(base.training_sessions_y1, 'sessione', 'sessioni')} di formazione per tutti`);
  const ergo = base.y1 ? quantitaErgonomia(base.y1.ergonomia) : null;
  if (ergo) out.push(ergo);
  out.push('4 report nell\'anno (Attivazione, due review, annuale)');
  return out;
}
...
// restano quelli approvati. Sotto le 12 voci: le quantità del primo anno e un solo numero,
// l'investimento annuo (mai valori per voce).
export function cosaComprendeMarkdown({ quantita = [], investimento = null } = {}) {
  const voci = VOCI_PROGRAMMA.map(v => `- **${v.n} · ${v.nome}.** ${v.cliente}${v.nota ? ` ${v.nota}` : ''}`).join('\n');
  const q = quantita.length ? `\n\n**Il vostro programma nel primo anno:** ${quantita.join(' · ')}.` : '';
  const inv = investimento ? `\n\n**Investimento annuo (Anno 1): ${eur(investimento)}.**` : '';
