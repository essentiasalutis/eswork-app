...
export async function getProComplianceOverview() {
  ...
  return pros.map(p => {
    ...
    const { qualificationOk, missing, complete } = complianceState(presentTypes);
    // Idoneità all'assegnazione a un'azienda (conformità completa + RC non scaduta).
    const { blocked: assignBlocked, reasons: assignReasons } = assignmentEligibility(presentTypes, rcStatus);
    return {
      professional: p,
      docs: docMap,
      rcStatus,
...
export async function getProAssignmentEligibility(professional_id) {
  ...
  return assignmentEligibility(presentTypes, rcStatus);
}
