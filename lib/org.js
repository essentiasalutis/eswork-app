...
export async function getOrgParams() {
  const p = {
    ...
    offertaGiorniA: 10, // validità dell'offerta per il binario A; il B non scade (decisione Enrico)
  ...
  try {
    ...
    for (const r of data || []) {
      ...
      else if (r.key === 'offerta_giorni_a' && Number.isInteger(num) && num >= 1 && num <= 90) p.offertaGiorniA = num;
