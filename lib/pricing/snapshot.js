...
// GUARDIA (walk 2-bis). Il cassetto dello snapshot è UNO per azienda, non uno per
// prodotto. La Stima del pacchetto non ha forbice: sovrascrivendo cancellerebbe la
// promessa del programma completo, e il Report scivolerebbe in silenzio sul
// ricalcolo live (prezzo giusto, ma confrontato con una forbice mai presentata).
// Qui la forbice promessa viene conservata a parte, con i SUOI inputs/v2Params —
// mai mescolata con quelli della Stima pacchetto.
function preservaForchetta(nuovo, existing) {
  if (nuovo.forchetta) return nuovo;              // Stima programma: è lei la promessa
  const carry = existing && (existing.forchetta ? existing : existing.programma_completo);
  if (!carry || !carry.forchetta) return nuovo;
  const { programma_completo: _annidato, ...pulito } = carry;
  return { ...nuovo, programma_completo: pulito };
}
...
export async function writeStimaSnapshotIfOpen(client_id, snapshotIn) {
  const fm = await getFirstMeeting(client_id);
  const existing = fm?.stima_snapshot || null;
  // Chiusa se esiste un Report OPPURE se lo snapshot è già timbrato (frozen_at).
  if ((existing && existing.frozen_at) || await isChainClosed(client_id)) {
    return { written: false, frozen: true, snapshot: existing };
  }
  const snapshot = preservaForchetta(snapshotIn, existing);
  if (fm) {
    ...
    // Nessuna scheda letta: se però ce ne sono (doppie), non se ne crea un'altra (21/9).
    const { count } = await supabase.from('first_meetings').select('id', { count: 'exact', head: true }).eq('client_id', client_id);
    if (count > 0) throw new Error(`Questa azienda ha ${count} schede del colloquio invece di una: la Stima non si impegna finché non se ne tiene una sola.`);
    const { error } = await supabase.from('first_meetings')
    ...
    if (error && error.code === '23505') throw new Error('La scheda del colloquio è stata creata da un altro salvataggio in questo istante: riprova.');
    if (error) throw new Error(error.message);
  }
...
// Resolver per i documenti del PROGRAMMA COMPLETO (Report + banner Offerta):
// ritorna lo snapshot che porta la forbice promessa — quello corrente se è del
// programma, altrimenti quello conservato da preservaForchetta. Null → path live.
export function getForchettaSnapshot(firstMeeting) {
  const snap = firstMeeting?.stima_snapshot || null;
  if (!snap) return null;
  if (snap.forchetta) return snap;
  if (snap.programma_completo && snap.programma_completo.forchetta) return snap.programma_completo;
  return null;
}
