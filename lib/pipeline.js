...
// pausa e non conta; chi ha già firmato nemmeno.
...
// data entro `giorni` (quelle già passate restano in cima, in rosso), i secondi incontri
// (presentazione del Report) e i check-up ancora aperti che chiudono nella settimana. `checkups` arriva già filtrato (aperti,
...
export function agendaSettimana({ clients = [], checkups = [], oggi, giorni = 7 }) {
  ...
  for (const c of clients) {
    ...
    const base = { client_id: c.id, cliente: c.name, is_demo: !!c.is_demo };
    ...
    // Secondo incontro (presentazione del Report): da oggi ai prossimi giorni, per chi non ha ancora deciso.
    if (isAperto(s) && isYmd(c.secondo_incontro_il) && c.secondo_incontro_il >= oggi && c.secondo_incontro_il <= limite) out.push({ ...base, tipo: 'incontro', data: c.secondo_incontro_il, scaduto: false });
  }
  for (const a of checkups) {
    if (isYmd(a.chiude_il) && a.chiude_il >= oggi && a.chiude_il <= limite) out.push({ client_id: a.client_id, cliente: a.cliente, is_demo: !!a.is_demo, tipo: 'checkup', data: a.chiude_il, scaduto: false });
  }
  const peso = { incontro: 0, ricontatto: 1, offerta: 2, checkup: 3 };
