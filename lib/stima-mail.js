...
export function testoMailStima({ azienda, referente, forchetta, pacchetto, url }) {
  ...
  const prezzo = pacchetto
    ? `Il Pacchetto d'ingresso ha un investimento di ${eur(pacchetto.price)} per 12 mesi (${DICITURA_IVA_BREVE}).`
    : `Sulla base dei vostri numeri, il programma si colloca tra ${eur(forchetta?.min?.price_y1)} e ${eur(forchetta?.max?.price_y1)} all'anno (${DICITURA_IVA_BREVE}). Il preventivo preciso esce dal check-up, sui vostri dati reali, e resta all'interno di questa forbice.`;
