...
// Tempo richiesto a una persona nel primo anno di programma, in ore.
// Composizione (la stessa usata dal prezzo, lib/pricing/v2.js):
//   · in trattamento: un ciclo di sedute + la formazione del primo anno;
//   · tutti gli altri: la formazione del primo anno.
// Arrotondato PER ECCESSO all'ora (Enrico, 17/9): meglio promettere un po' più del
// tempo che si chiede che sembrare precisi e sbagliare per difetto. Nel testo va «circa».
export function oreRichiestePrimoAnno(p = PROTOCOLLO) {
  ...
  const ciclo = (p.sedute_per_ciclo * p.durata_seduta_min) / 60;
  return {
    trattamento: Math.ceil(ciclo + formazione),
    altri: Math.ceil(formazione),
    esatte: { trattamento: ciclo + formazione, altri: formazione },
  };
}

// «circa 4 ore nel primo anno di programma per chi è in trattamento e 2 ore per tutti gli altri»
export function fraseTempoRichiesto({ breve = false } = {}) {
  const o = oreRichiestePrimoAnno();
  const ore = (n) => `${n} ${n === 1 ? 'ora' : 'ore'}`;
  return `circa ${ore(o.trattamento)} nel primo anno di programma per chi è in trattamento${breve ? ',' : ' e'} ${breve ? o.altri : ore(o.altri)} per tutti gli altri`;
}
