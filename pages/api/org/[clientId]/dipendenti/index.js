...
export default requireAuth(async function handler(req, res) {
  ...
  const { nome, data_ingresso, matricola, identificativo_hr, straordinario, area } = req.body || {};
  if (!nome || !String(nome).trim()) return res.status(400).json({ error: 'nome obbligatorio' });
  try {
    return res.json(await aggiungiDipendente(clientId, { nome, data_ingresso, matricola, identificativo_hr, straordinario, area }));
