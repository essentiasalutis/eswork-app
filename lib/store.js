...
export async function getProComplianceOverview() {
  ...
  return pros.map(p => {
    ...
    let rcStatus = 'missing';
    if (rc) {
      if (!rc.expiry_date) rcStatus = 'no_expiry';
      else {
        const e = Date.parse(rc.expiry_date);
        rcStatus = e < now ? 'expired' : e <= in30 ? 'expiring' : 'valid';
      }
    }
    const docMap = {};
    ...
    return {
      ...
      rcBlocking: rcStatus === 'expired', // Art. 7.2: RC scaduta → operatività sospesa
    };
  });
}
