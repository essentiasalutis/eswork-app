...
// Default del codice. I listini della formazione possono essere sovrascritti da
// pricing_settings (v2, Listino) e per azienda (clients.listino_*). Finestra e soglie
// di recupero invece sono regole del protocollo (Art. 5-bis c. 5): solo da qui.
// Nessun override per azienda della soglia (Enrico, 17/9): vale la fascia del contratto.
export const ORG_PARAMS = {
  // Regole contrattuali (Art. 5-bis c. 5): dal protocollo, non modificabili.
  finestra_mesi: PROTOCOLLO.recupero_finestra_mesi,
  soglia_fasce: PROTOCOLLO.recupero_soglie,
...
// Trigger: parte se la coda raggiunge la soglia della fascia OPPURE alla scadenza della finestra dall'ingresso del primo in coda.
...
export function propostaRecupero(coda, client, today, orgParams = null) {
  ...
  const soglia = sogliaDefaultPerFascia(client && client.employees, op.sogliaFasce);
