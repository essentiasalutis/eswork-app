...
export async function aggiornaDemo({ nome, dipendenti, settore }) {
  ...
  if (fm) {
    ...
    const nuovo = {
      ...
      step2: { ...(data.step2 || {}), sedi: [{ nome: 'Sede principale', employees: n }] },
    };
    const { error: e2 } = await supabase.from('first_meetings').update({ data: nuovo, employees: n, sector: SETTORI[settore], updated_at: new Date().toISOString() }).eq('id', fm.id);
