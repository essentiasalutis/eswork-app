...
export function valutaSconto({ prezzoBase, prezzoScontato, costo, sogliaPct = 0.4, conferma = false, motivo = null, controllaMotivo = true }) {
  ...
  if (m.marginePct < soglia && !conferma) {
    return esito(false, 'serve_conferma', `Margine del ${pctIt(m.marginePct)} (${eurIt(m.margineEur)}): sotto la soglia del ${pctIt(soglia)}. Conferma per registrarlo.`, dati);
  }
  return esito(true, 'valido', m.marginePct < soglia ? `Margine del ${pctIt(m.marginePct)}, sotto la soglia del ${pctIt(soglia)}: confermato.` : `Margine del ${pctIt(m.marginePct)}.`, { ...dati, sottoSoglia: m.marginePct < soglia });
}
