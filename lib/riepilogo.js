...
import { fraseTempoRichiesto } from './protocollo.mjs';
import { DICITURA_IVA_BREVE } from './iva.mjs';
...
// Kit di comunicazione interna per i dipendenti — momento «in valutazione».
// UN SOLO REGISTRO, istituzionale (Enrico, 13/9): in una micro-impresa suona formale
// ma non stona, mentre il «ciao a tutti» in un'azienda strutturata sì. Due varianti
// erano una scelta in più a ogni invio per un vantaggio marginale. La variante diretta
// è stata rimossa: un testo che nessuna schermata può più produrre invecchia in
// silenzio (resta nella storia di git, se un giorno servisse).
export function testoKit({ link, scadenza, firma } = {}) {
  return {
...
Il check-up resta aperto ${finoAl(scadenza)}. Chi non compila entro tale data non rientra nel check-up: senza le sue risposte non è possibile includerlo nei percorsi individuali — trattamento o prevenzione — che l'azienda deciderà eventualmente di attivare. La formazione collettiva su postura ed ergonomia resta aperta a tutti.
...
// Invito al check-up DOPO la firma (quando si firma prima del check-up). Testo di Enrico
...
export function testoKitCheckupDopoFirma({ link, scadenza, firma } = {}) {
  // Registro unico, istituzionale — come il kit «in valutazione». Il consenso è
  // anticipato nel testo (Enrico 11/9: evita «ma questo cosa mi sta facendo firmare?»):
  // vero in piattaforma, la pagina del check-up chiede informativa e consenso art. 9
  // prima delle domande.
  return {
      oggetto: `Check-up muscolo-scheletrico aziendale — richiesta di compilazione ${entroIl(scadenza)}`,
      corpo: `Gentili colleghe e colleghi,

l'azienda ha attivato un programma dedicato alla prevenzione e al trattamento dei disturbi muscolo-scheletrici (lombalgie, cervicalgie, disturbi legati alla postura e alla movimentazione).

Il primo passo è un check-up: un questionario di meno di 5 minuti, compilabile da smartphone al link indicato di seguito. Il programma viene costruito sui risultati di questo check-up.

Riservatezza. Il questionario è riservato: le risposte individuali non sono visibili all'azienda in alcuna forma. L'azienda riceve esclusivamente dati aggregati di popolazione, con soglie che impediscono l'identificazione dei singoli. Il check-up è gestito da Essentia Salutis (Torino), rete di professionisti sanitari, in qualità di titolare autonomo del trattamento. Prima della compilazione vi sarà richiesto il consenso al trattamento dei dati relativi alla salute, con l'informativa completa.

${link || ''}

Il check-up resta aperto ${finoAl(scadenza)}. Chi non compila entro tale data non rientra nella rilevazione iniziale. Il programma resta comunque a disposizione di tutti, e sarà possibile segnalare disturbi anche in seguito.

La compilazione è volontaria e non comporta alcun costo.
...
// «Bozza»: il testo lo manda l'azienda, con le sue parole se preferisce. Chiamarlo
// «testo da inoltrare» suonava come un compito assegnato al referente (Enrico, 14/9).
export function bloccoKit(kit) {
  return `——————————————
BOZZA DI TESTO DA INOLTRARE AI DIPENDENTI
...
export function testoRiepilogo({ referente, contesto, forchetta, urlStima, link, scadenza, secondoIncontro, conLettera = false, kit } = {}) {
  ...
  // La Lettera è un documento OPZIONALE: la riga compare solo se l'ho accesa per
  // questa azienda, non più in base a un attributo del cliente (Enrico, 13/9).
  const lettera = conLettera
...
${voci}
Tempo richiesto: ${fraseTempoRichiesto()}.

Stima di investimento. Sulla base dei vostri numeri, il programma si colloca tra ${eur(forchetta?.min)} e ${eur(forchetta?.max)} all'anno (${DICITURA_IVA_BREVE}). Il preventivo preciso esce dal check-up, sui vostri dati reali, e resta all'interno di questa forbice.
${documento}
...
- Trovate qui sotto una bozza di testo da inoltrare: potete usarla così com'è o adattarla.
