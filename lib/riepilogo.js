...
export function testoKitCheckupDopoFirma({ variante = 'A', link, scadenza, firma } = {}) {
  // Variante B (istituzionale): dal kit B con gli stessi ritocchi, "a disposizione di tutti",
  // senza "L'esito della valutazione…" (dopo la firma non c'è più una valutazione) e con il
  // consenso anticipato (Enrico 11/9: evita "ma questo cosa mi sta facendo firmare?"). Vero
  // in piattaforma: la pagina del check-up chiede informativa e consenso art. 9 prima delle domande.
  if (variante !== 'A') {
    return {
      oggetto: `Check-up muscolo-scheletrico aziendale — richiesta di compilazione ${entroIl(scadenza)}`,
      corpo: `Gentili colleghe e colleghi,
