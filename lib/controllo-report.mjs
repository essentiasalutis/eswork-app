...
const VIETATI = [
  ...
  [/(?<!persone |d'|d’|di )\batte[sz][oaie]\b/i, 'valore «atteso» non fornito'],
  ...
  [/(senza|nessuna|zero) (liste?|coda) d(i |'|’)attesa/i, 'affermazione sulla lista d\'attesa non fornita dai dati'],
...
// `scadenza` (ms): la funzione su Vercel ha un tempo massimo (60 s). La riscrittura
// parte solo se, stimandola lunga quanto la prima chiamata, finisce prima della
// scadenza; altrimenti il report si salva «da rivedere» (il 18/9 il Report di
// Attivazione con due chiamate da 4000 token andava in 504).
export async function generaConControllo(chiedi, prompt, { scadenza = Infinity, adesso = () => Date.now() } = {}) {
  const inizio = adesso();
  const primo = await chiedi([{ role: 'user', content: prompt }]);
  if (primo.troncato) return { ...primo, problemi: [] };
  const durata = adesso() - inizio;
  let { testo } = primo;
  let problemi = controllaTesto(testo, { dati: prompt });
  if (problemi.length && adesso() + durata * 1.2 < scadenza) {
