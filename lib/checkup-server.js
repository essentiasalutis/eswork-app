...
import { isFirmato as firmatoStage } from './pipeline';

// "Accettato" = contratto firmato. Regola nella fonte unica lib/pipeline.js (legge anche
// i vecchi valori 'active' e 'won').
export function isFirmato(client) {
  return !!client && firmatoStage(client.pipeline_stage);
}
...
// Check-up ancora aperti che chiudono entro `limite` (per l'agenda della dashboard).
// Senza v51 nessuna data: lista vuota, nessun errore.
export async function getCheckupInChiusura({ oggi, limite }) {
  try {
    const r = await supabase.from('assessments')
      .select('id, client_id, created_at, chiude_il, clients(name, is_demo, binario)')
      .eq('status', 'active').gte('chiude_il', oggi).lte('chiude_il', limite);
    if (r.error) return [];
    const out = [];
    for (const a of r.data || []) {
      if (await reportAttivazioneDopo(a.client_id, a.created_at).catch(() => true)) continue;
      const c = a.clients || {};
      out.push({ client_id: a.client_id, cliente: c.name || a.client_id, is_demo: !!c.is_demo, binario: c.binario || null, chiude_il: a.chiude_il });
    }
    return out;
  } catch (_) { return []; }
}
