...
export const ANNI_CONSERVAZIONE = CONFIG.retention_years || 10;
...
export async function vincoloConservazioneAzienda(clientId) {
  ...
  return {
    ...
    finoAl: ultimo,
    messaggio: `Cancellazione dell'azienda non eseguita: ${bloccati.length === 1 ? '1 paziente ha' : `${bloccati.length} pazienti hanno`} documentazione clinica ancora nel periodo di conservazione di ${ANNI_CONSERVAZIONE} anni (l'ultima fino al ${new Date(ultimo).toLocaleDateString('it-IT', { timeZone: 'Europe/Rome' })}). Va conservata fino ad allora.`,
  };
}
