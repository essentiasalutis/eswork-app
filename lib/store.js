...
export async function getWaitlistByClient(client_id) {
  const { data, error } = await supabase
    .from('waitlist')
    .select('*, patients(first_name, last_name, level, computed_level)')
    .eq('client_id', client_id)
    .eq('status', 'pending')
...
export async function getPatientsForMinicheckInvite(checkpoint_type) {
  // T3: ~90 giorni dopo assessment_completed_at
  // T6: ~180 giorni dopo assessment_completed_at
  ...
  const { data, error } = await supabase.from('patients')
    ...
    .gte('assessment_completed_at', windowStart)
    .lte('assessment_completed_at', windowEnd);
  if (error) throw error;
  // Filtra chi non ha già un minicheck per questo checkpoint
  ...
  return (data || []).filter(p => !alreadyChecked.has(p.id));
}
