...
//   cartella clinica.
// Livello B — cartella clinica di dettaglio (anamnesi, note, sessioni): visibile
//   SOLO all'osteopata con assigned_professional_id = quel paziente.
//   assigned_professional_id si popola alla pre-validazione: finché è null non esiste
//   cartella da proteggere, quindi si ricade sul Livello A per non bloccare i flussi.
...
export async function proCanAccessPatientClinical(proId, patient) {
  ...
  // Deve comunque avere un'assegnazione ATTIVA all'azienda del paziente: se il
  // professionista viene disabilitato da quell'azienda, perde anche le cartelle.
  const clientOk = await proCanAccessClient(proId, patient.client_id);
  if (patient.assigned_professional_id) {
    return patient.assigned_professional_id === proId && clientOk;
  }
  // Nessun assegnatario ancora: ricade sull'accesso di livello azienda (Livello A)
  return clientOk;
}
...
export async function getReferralCodesByClient(client_id) {
  ...
  let { data, error } = await supabase
    .from('referral_codes')
    .select('*, referral_uses(id, patient_name, patient_phone, patient_email, preferred_when, used_at, voucher_code, status, amount, redeemed_at)')
    .eq('client_id', client_id)
    .order('created_at', { ascending: false });
  if (error) {
...
export async function getAllReferralCodes() {
  let { data, error } = await supabase
    .from('referral_codes')
    .select('*, clients(id, name), referral_uses(id, used_at, patient_name, status, amount)')
    .order('created_at', { ascending: false });
  if (error) {
    ({ data, error } = await supabase
      .from('referral_codes')
