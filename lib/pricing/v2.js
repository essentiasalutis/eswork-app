...
import { DEFAULTS_V2 } from './v2-defaults.mjs';
import { conProtocollo, PROTOCOLLO, oreRichiestePrimoAnno } from '../protocollo.mjs';
import { verificaTariffe } from '../tariffe.mjs';
export { DEFAULTS_V2 };


// Le regole del protocollo vincono sempre sugli override: ciò che si vende è ciò che si eroga.
const P = (v2Params) => conProtocollo({ ...DEFAULTS_V2, ...(v2Params || {}) });

// Ergonomia a TRE voci:
//   1. ufficio   — ogni dipendente d'ufficio: minuti a persona sulla sua scrivania;
//   2. studio    — ogni postazione tipo di reparto: FORFAIT (un deliverable, non
//                  un tempo: analisi, misure, indicazioni scritte);
//   3. addetti   — ogni addetto di reparto viene formato sulla postazione già
//                  studiata. Senza questa voce si pagava l'analisi e chi ci lavora
//                  non imparava come starci: la parte che produce il risultato.
// Il tempo dello studio resta, ma solo per il minimo ore e la pianificazione.
// Default motore (input assenti): tutta la popolazione in ufficio, zero reparto.
// NB compatibilità: al momento del cambio non esisteva alcuno snapshot di Stima
// in produzione (verificato 2026-09-11), quindi nessuna promessa congelata si muove.
const r2 = (x) => Math.round(x * 100) / 100; // centesimi: niente derive floating-point

function computeErgonomia({ nUfficio, nAddetti, nPostazioni, rates, p }) {
  const nU = Math.max(0, parseInt(nUfficio) || 0);
  const nA = Math.max(0, parseInt(nAddetti) || 0);
  const nP = Math.max(0, parseInt(nPostazioni) || 0);
  const uffH = (nU * p.ergonomia_minuti_persona) / 60;
  const addH = (nA * p.ergonomia_minuti_addetto) / 60;
  const studioH = (nP * p.ergonomia_minuti_postazione) / 60;
  const hours = uffH + addH + studioH;             // ore in sede: minimo + pianificazione
  const studioSell = nP * p.ergonomia_forfait_postazione;
  return {
    label: 'Consulenza ergonomico-posturale',
    detail: `${nU} persone (ufficio) × ${p.ergonomia_minuti_persona}′ + ${nA} addetti (reparto) × ${p.ergonomia_minuti_addetto}′ + ${nP} postazioni tipo × €${p.ergonomia_forfait_postazione}`,
    nUfficio: nU, nAddetti: nA, nPostazioni: nP,
    hours,
    sell: r2((uffH + addH) * rates.sportello_sell + studioSell),
    cost: r2(hours * rates.sportello_cost),        // l'osteopata si paga a tempo, anche lo studio
...
// Lettura UNICA degli input ergonomia salvati nel colloquio (first_meetings.data.step2),
// usata da Report e Offerta (la scheda e la Stima passano già numeri espliciti).
// Dal 2026-09-11 il colloquio salva SEMPRE numeri: il campo ufficio arriva
// precompilato e "vuoto" vale 0. Null resta solo nei colloqui salvati prima: si
// legge come li leggeva allora la Stima (ufficio assente = tutta la popolazione),
// così Report e Stima non danno mai due letture diverse dello stesso colloquio.
export function ergonomiaDaColloquio(s2, n) {
  const num = (v) => (v === null || v === undefined) ? null : Math.max(0, parseInt(v, 10) || 0);
  const u = num(s2 && s2.ergonomia_ufficio);
  return {
    nUfficio: u === null ? (parseInt(n) || 0) : u,
    nAddetti: num(s2 && s2.ergonomia_addetti) ?? 0,
    nPostazioni: num(s2 && s2.ergonomia_postazioni) ?? 0,
  };
}

// Voci di un anno v2. includeErgonomia=false per Anno 2+ (a consumo).
function computeYearV2({ n, l1, l2, groups, modules, rates, p, vatExempt, ergonomia, includeErgonomia }) {
  ...
  const prevalidations = {
    ...
    detail: `${l1} × ${PROTOCOLLO.durata_prevalidazione_min}′`,
    count: l1,
  ...
  // f) Programma, misurazione e regia — ogni anno di programma, mai col buffer.
  const quotaSell = r2((p.quota_programma_fissa || 0) + n * (p.quota_programma_per_dipendente || 0));
  const quota = {
    label: 'Programma, misurazione e regia',
    detail: `€${p.quota_programma_fissa || 0} + ${n} dipendenti × €${p.quota_programma_per_dipendente || 0}`,
    sell: quotaSell,
    cost: r2(quotaSell * (p.quota_programma_costo_pct || 0)),
  };

  const base = ergo ? [sportello, prevalidations, prevention, training, ergo] : [sportello, prevalidations, prevention, training];
  const items = quota.sell > 0 ? [...base, quota] : base;
  ...
  return {
    items,
    sportello, prevalidations, prevention, training, ergonomia: ergo, quota,
...
export function calculatePricing(arg) {
  ...
  // Nessun ripiego sulle tariffe standard (Enrico, 21/9): senza tariffe complete il
  // calcolo si ferma e dice quali mancano (lib/tariffe.mjs).
  const rates = verificaTariffe(a.rates);
  const groups = a.groups != null ? a.groups : Math.ceil(n / p.capienza_aula) || 1;
  const ergonomia = { nUfficio: a.ergonomia?.nUfficio ?? n, nAddetti: a.ergonomia?.nAddetti ?? 0, nPostazioni: a.ergonomia?.nPostazioni ?? 0 };

  const base = { n, l1, l2, groups, rates, p, vatExempt: a.vatExempt, ergonomia };
  ...
  return {
    ...
    // Tempo per persona nel primo anno: dal protocollo, tre voci, per eccesso (come i testi).
    hours_treated: oreRichiestePrimoAnno().trattamento,
    hours_prevention: oreRichiestePrimoAnno().prevenzione,
    hours_untreated: oreRichiestePrimoAnno().altri,
...
export function validatePacchetto({ employees, pricingVersion, v2Params } = {}) {
  ...
  // Nessun tetto di popolazione (Enrico, 12/9): il pacchetto è un prodotto a sé —
  // check-up, formazione e ergonomia, senza presa in carico clinica — e si può
  // proporre a un'azienda di qualsiasi dimensione. Il parametro soglia_ingresso
  // del Listino non è più usato.
  if (n <= 0) {
    return { ok: false, motivo: 'popolazione non indicata' };
  }
  return { ok: true };
}
...
export function calculatePacchetto({ n, groups, rates, vatExempt, v2Params, ergonomia } = {}) {
  ...
  const ergo = computeErgonomia({ nUfficio: ergonomia?.nUfficio ?? N, nAddetti: ergonomia?.nAddetti ?? 0, nPostazioni: ergonomia?.nPostazioni ?? 0, rates: r, p });
  // Check-up: la parte per dipendente della quota «Programma, misurazione e regia»
  // (dal 21/9 sostituisce il vecchio parametro «check-up €15/dipendente»). Niente
  // quota fissa: il Pacchetto resta la porta d'ingresso economica.
  const assessmentSell = r2(N * (p.quota_programma_per_dipendente || 0));
  const assessment = {
    label: 'Check-up completo',
    detail: `${N} dipendenti dichiarati × €${p.quota_programma_per_dipendente || 0} (quota per dipendente) — consensi e flussi clinici identici al programma completo`,
    sell: assessmentSell,
    cost: r2(assessmentSell * (p.quota_programma_costo_pct || 0)),
  };
