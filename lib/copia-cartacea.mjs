...
// Documenti per cui la copia su carta è SOSPESA, con il perché (Enrico, 17/9).
// L'informativa estesa in vigore indica il consenso come base giuridica ma non
// contiene una formula di consenso da firmare: la firma su carta non proverebbe
// un consenso che il paziente non ha prestato. In piattaforma il consenso art. 9
// è nella dichiarazione della firma cumulativa (per ora scritta nel codice: le
// dichiarazioni vanno nell'archivio dopo la revisione dell'avvocato).
// Si riattiva togliendo la voce, quando la nuova versione contiene la formula.
export const CARTA_SOSPESA = {
  privacy_extended: 'carta_sospesa_informativa',
};

export function cartaSospesa(tipo) {
  return Object.prototype.hasOwnProperty.call(CARTA_SOSPESA, tipo);
}
...
export const MESSAGGI = {
  ...
  documento_non_valido: 'Questo documento non si carica su carta.',
  carta_sospesa_informativa: 'L\'informativa estesa per ora non si può caricare su carta: il testo in vigore non contiene una formula di consenso al trattamento dei dati di salute (art. 9), quindi una firma su carta non proverebbe un consenso che il paziente non ha prestato. Falla firmare in piattaforma con la firma cumulativa, che raccoglie il consenso. Il caricamento su carta tornerà disponibile con la nuova versione del testo.',
