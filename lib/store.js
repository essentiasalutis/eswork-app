...
import { complianceState, assignmentEligibility, DOC_TYPES } from './pro-docs';
import { statiAccordo } from './accordo-server';
import { finestraAnno, nellAnno, MAX_AUTOSEGNALAZIONI } from './anno-programma.mjs';
...
// care_token = credenziale che apre la cartella clinica del paziente: DEVE essere
// crittograficamente forte (mai Math.random). 192 bit da crypto.randomBytes.
// (Il percorso pro generate-care-token usa già randomBytes; qui si allinea /self-declare.)
export function generateCareToken() {
  return crypto.randomBytes(24).toString('hex');
}

// Codici che compaiono in URL PUBBLICI (link questionario): devono essere
// crittograficamente forti. Math.random è PREDICIBILE (V8 usa xorshift128+: da
// pochi output si ricostruisce lo stato e si derivano i successivi) → chi vede un
// solo codice potrebbe indovinare quelli generati vicino. crypto.randomInt no.
const CODE_ALPHABET = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';

export function randomCode(len = 10, alphabet = CODE_ALPHABET) {
  let out = '';
  for (let i = 0; i < len; i++) out += alphabet[crypto.randomInt(alphabet.length)];
  return out;
}

// 10 caratteri su 62 simboli ≈ 8.4e17 combinazioni: non enumerabile, e resta
// corto abbastanza da stare in un link. I codici già emessi restano validi.
...
export async function ensureClientAssessmentShareCode(client_id) {
  ...
  // Genera codice se mancante (stesso generatore forte del share_code)
...
// Quante schede del colloquio ha un'azienda. getFirstMeeting ne vuole UNA: con due o
// più non ne legge nessuna, e fino al 21/9 il ripiego sulle tariffe standard lo
// nascondeva (Industrie Lisa, 15 schede). Serve a dire il perché, invece di «manca».
export async function contaSchedeColloquio(client_id) {
  const { count } = await supabase.from('first_meetings').select('id', { count: 'exact', head: true }).eq('client_id', client_id);
  return count || 0;
}
export const messaggioSchedeDoppie = (n, azienda) =>
  `${azienda || 'Questa azienda'} ha ${n} schede del colloquio invece di una: la piattaforma non sa quale usare e non ne legge nessuna. Va tenuta una sola scheda prima di calcolare Offerta e report.`;

// Una sola scheda per azienda (21/9). Prima: due salvataggi ravvicinati su un'azienda
// nuova creavano due righe; da lì getFirstMeeting non ne leggeva nessuna e ogni
// salvataggio ne aggiungeva un'altra (Industrie Lisa, 15). Ora:
//   · con più schede NON si aggiorna né si crea: si ferma e lo dice (aggiornarle tutte
//     insieme cancellerebbe le differenze da cui si sceglie quale tenere);
//   · la corsa fra due salvataggi la chiude l'indice unico su client_id (v77): chi
//     arriva secondo trova la riga e la aggiorna.
export class SchedeDoppie extends Error {
  constructor(n, azienda) { super(messaggioSchedeDoppie(n, azienda)); this.name = 'SchedeDoppie'; this.n = n; }
}

export async function upsertFirstMeeting(client_id, fields) {
  const n = await contaSchedeColloquio(client_id);
  if (n > 1) throw new SchedeDoppie(n);
...
export async function createSelfDeclaredPatient({ client_id, first_name, last_name, email, phone, location, wants_to_be_contacted }) {
  ...
  const care_token = generateCareToken();
  const { data, error } = await supabase
    .from('patients')
    .insert({
      id: generateId('pat'),
      client_id,
      first_name: first_name || 'Nome non indicato',
      last_name: last_name || '',
      email: email || null,
      phone: phone || null,
...
export async function getSessionsForClient(client_id) {
  ...
  const { data, error } = await supabase
    .from('sessions')
    .select('id, patient_id, cycle_id, session_number, nrs_pre, nrs_post, date, closed_at, patients(first_name, last_name, level)')
    .eq('client_id', client_id)
...
export function buildVoucherCode() {
  ...
  const s = randomCode(8, A); // sorgente crypto, formato leggibile invariato
...
// ─── Azienda demo permanente (v78) ───────────────────────────────────────────
// I suoi dati vivono il tempo di un convegno: non entrano negli elenchi di
// conformità, ri-stratificazioni, conservazione e registro (Enrico, 21/9).
// Senza la v78 la colonna non esiste: nessuna esclusione, nessun errore.
export async function idDemoPermanente() {
  const { data, error } = await supabase.from('clients').select('id').eq('demo_permanente', true).maybeSingle();
...
export async function getAllPatients() {
  const [{ data, error }, demo] = await Promise.all([
    supabase.from('patients')
      .select('id, first_name, last_name, client_id, level, prevention_eligible, clients(name)')
      .order('created_at', { ascending: false }),
    idDemoPermanente(),
  ]);
  if (error) throw error;
  return (data || []).filter(p => !demo || p.client_id !== demo);
}
...
// Integrazioni dell'anamnesi (v71): righe solo in aggiunta, mai modificate.
export async function getIntegrazioniAnamnesi(patient_id) {
  const { data, error } = await supabase
    .from('anamnesi_integrazioni')
    .select('id, campo, valore_prima, valore_dopo, motivo, professional_id, gruppo, creato_il')
    .eq('patient_id', patient_id)
...
export async function insertIntegrazioniAnamnesi(righe) {
  const { data, error } = await supabase.from('anamnesi_integrazioni').insert(righe).select();
  if (error) throw error;
  return data || [];
}
...
export async function getDocumentComplianceByClient() {
  const { data, error } = await supabase
    .from('patient_documents')
    .select('client_id, patient_id, type, status, testo_legale_id, modalita, clients(name)');
  if (error) throw error;
  return data || [];
}
...
export async function logAccess({ professional_id, action, patient_id = null, ip = null, user_agent = null, details = null }) {
  ...
  if (error) {
    ...
    const r2 = await supabase.from('access_logs').insert(base);
    if (r2.error) return { ok: false, motivo: error.message };
    // Riga scritta, ma SENZA il collegamento al record (es. riferimento non valido):
    // è un registro degradato e chi chiama deve saperlo, non riceverlo come successo.
    return { ok: true, ridotta: true, motivo: error.message };
  }
  // Chi chiama deve poter sapere se la riga è stata scritta: un registro che smette
  // di scrivere in silenzio è peggio di uno assente (Enrico, 12/9).
  return { ok: true };
}
...
export async function getAccessLogs(limit = 200) {
  ...
  const demo = await idDemoPermanente();
  return (data || []).filter(r => !demo || !r.patients || r.patients.client_id !== demo);
}
...
// (ultima attività + PROTOCOLLO.anni_conservazione) e SEGNALA i dati oltre il termine,
// perché il titolare li cancelli in modo validato. L'ancora è la più recente fra:
// ultimo ciclo chiuso, completamento assessment, creazione del record.
export async function getRetentionReview() {
  const years = PROTOCOLLO.anni_conservazione;
  const [{ data: tutti, error }, demo] = await Promise.all([
    supabase.from('patients')
  ...
  const data = (tutti || []).filter(p => !demo || p.client_id !== demo);
...
export async function getProComplianceOverview() {
  ...
  // Accordo sul trattamento dei dati (v66). Se la lettura fallisce, l'esito manca
  // e la conformità blocca con «verifica non disponibile».
  const accordi = await statiAccordo(pros.map(p => p.id)).catch(() => ({}));

  return pros.map(p => {
    const pd = byPro[p.id] || {};
    const rc = pd.rc_policy || null;
    const { status: rcStatus, days: rcDays } = rcStatusFrom(rc);
    // docMap su TUTTO l'enum (incl. qualification_diploma + albo): la vista admin
    // legge i due sotto-slot per mostrare QUALE qualifica è presente.
    const docMap = {};
    ...
    // Conformità: identità + qualifica(almeno uno) + RC + contratto (no quietanza).
    const presentTypes = DOC_TYPES.filter(t => pd[t]);
    const accordo = accordi[p.id];
    const { qualificationOk, missing, complete } = complianceState(presentTypes, accordo);
    // Idoneità all'assegnazione a un'azienda (conformità completa + RC non scaduta).
    const { blocked: assignBlocked, reasons: assignReasons } = assignmentEligibility(presentTypes, rcStatus, accordo);
    return {
      professional: p,
      docs: docMap,
      rcStatus,
      ...
      qualificationOk,
      missing,   // etichette dei requisiti mancanti (Identità/Qualifica/Polizza RC/Contratto)
      complete,
      assignBlocked,   // true = NON assegnabile a nuove aziende
      assignReasons,   // motivi del blocco (per il pop-up)
      rcSuspended: isRcSuspended(rcStatus), // Art. 7.4: nessuna copertura valida (scaduta/mancante)
      accordo: accordo ? {
        stato: accordo.stato, conforme: accordo.conforme, versione: accordo.versione || null,
        scadenza: accordo.scadenza || null, motivo: accordo.motivo || null,
        firme: (accordo.firme || []).map(f => ({ testo_legale_id: f.testo_legale_id, valore: f.valore, atto_at: f.atto_at })),
        file: (accordo.file || []).map(f => ({ id: f.id, versione: f.versione, caricato_il: f.caricato_il, caricato_da: f.caricato_da })),
      } : null,
    };
  });
}

// Idoneità all'assegnazione di UN professionista (usata dall'enforcement server-side
// nel PUT delle assegnazioni). Stessa logica della vista admin, sorgente unica.
export async function getProAssignmentEligibility(professional_id) {
  const docs = await getProDocuments(professional_id);
  const presentTypes = docs.map(d => d.doc_type);
  const rc = docs.find(d => d.doc_type === 'rc_policy') || null;
  const { status: rcStatus } = rcStatusFrom(rc);
  const accordo = (await statiAccordo([professional_id]).catch(() => ({})))[professional_id];
  return assignmentEligibility(presentTypes, rcStatus, accordo);
}
...
export async function getAllRestratAlerts() {
  const [{ data, error }, demo] = await Promise.all([
    supabase.from('restratification_alerts')
      .select('*, patients(first_name, last_name), clients(name)')
      .order('created_at', { ascending: false }),
    idDemoPermanente(),
  ]);
  if (error) throw error;
  return (data || []).filter(a => !demo || a.client_id !== demo);
}
...
// Tutte le pre-validazioni di una persona, dalla più recente (cartella e copia dei dati:
// con il secondo ciclo possono essere più d'una).
export async function getPreValidationsByPatient(patient_id) {
  const { data, error } = await supabase
    .from('pre_validations')
    .select('*')
    .eq('patient_id', patient_id)
...
// Auto-segnalazioni per ANNO DI PROGRAMMA dell'azienda (lib/anno-programma.mjs),
// non per anno solare: prima si contava dal 1° gennaio calcolato in UTC, che nella
// prima ora dell'anno italiano ripescava l'intero anno precedente.
export async function countSelfTriggersNellAnno(patient_id, finestra) {
  let q = supabase.from('self_triggers').select('id', { count: 'exact', head: true }).eq('patient_id', patient_id);
  if (finestra) {
    if (finestra.inizio) q = q.gte('created_at', finestra.inizio);
    q = q.lt('created_at', finestra.fine);
  }
...
// client: l'azienda del paziente (serve la data di avvio del programma).
export async function getSelfTriggerBudget(patient_id, client) {
  const finestra = finestraAnno(client?.data_avvio_programma || null);
  const used = await countSelfTriggersNellAnno(patient_id, finestra);
  return {
    used, max: MAX_AUTOSEGNALAZIONI, remaining: Math.max(0, MAX_AUTOSEGNALAZIONI - used),
    rinnovoIl: finestra ? finestra.rinnovoIl : null,
    rinnovoIlTesto: finestra ? dataIt(finestra.rinnovoIl, { day: 'numeric', month: 'long', year: 'numeric' }) : null,
  };
}
...
export async function insertReassessmentT12(fields) {
  const riga = { id: generateId('t12'), ...fields, created_at: new Date().toISOString() };
  let { data, error } = await supabase.from('reassessments_t12').insert(riga).select().single();
  // Tollerante alla v62 (colonna checkpoint): se manca, la rivalutazione si salva
  // comunque — meglio il dato senza etichetta che nessun dato.
  if (error && riga.checkpoint !== undefined) {
    const { checkpoint, ...senza } = riga;
    ({ data, error } = await supabase.from('reassessments_t12').insert(senza).select().single());
  }
  if (error) throw error;
  return data;
}
...
// ─── Capacità contrattuale trattamenti (buffer del protocollo) ────────────────
...
export async function getTreatmentCapacity(client_id) {
  ...
  // Percorsi dell'ANNO DI PROGRAMMA in corso (lib/anno-programma.mjs): il contratto
  // è annuale, e dal rinnovo la capacità riparte. Prima contava tutti gli anni.
  const finestra = finestraAnno(client.data_avvio_programma || null);
  const { data: cycles } = await supabase
    .from('treatment_cycles')
    .select('id, cycle_type, started_at, created_at')
    .eq('client_id', client_id);
  const used = (cycles || []).filter(c => (c.cycle_type || 'treatment') === 'treatment' && nellAnno(c.started_at || c.created_at, finestra)).length;
  ...
  return {
    ...
    anno_programma: finestra ? { numero: finestra.numero, dal: finestra.inizioGiorno, rinnovo_il: finestra.rinnovoIl } : null,
...
// Segnalazioni spontanee dei dipendenti (per il report del movimento a 3 mesi).
// Solo conteggi: nessun testo del disturbo esce da qui.
export async function getSelfTriggersByClient(client_id) {
  const { data, error } = await supabase
    .from('self_triggers')
...
export async function getMonitoringByClient(client_id) {
  const [{ data: cycles }, { data: checks }, { data: reass }] = await Promise.all([
    ...
    supabase.from('reassessments_t12')
      .select('patient_id, pgic, completed_at, created_at, checkpoint')
      .eq('client_id', client_id)
      .then(r => (r.error ? supabase.from('reassessments_t12')
        .select('patient_id, pgic, completed_at, created_at')
        .eq('client_id', client_id) : r)),   // pre-v62: la colonna non c'è ancora
  ]);
...
// checkpoint: 't12' (annuale, default storico) oppure 't6' (ri-fotografia a sei mesi).
// Le righe scritte prima della v62 non hanno la colonna e valgono come annuali.
export async function getReassessmentsT12ByClient(client_id, checkpoint = 't12') {
  const campi = 'patient_id, nmq_data, pgic, computed_level, completed_at, created_at, checkpoint';
  let { data, error } = await supabase
    .from('reassessments_t12')
    .select(campi)
    .eq('client_id', client_id)
    .order('created_at', { ascending: false });
  if (error) {  // pre-v62: la colonna non esiste ancora
    ({ data, error } = await supabase
      .from('reassessments_t12')
      .select('patient_id, nmq_data, pgic, computed_level, completed_at, created_at')
      .eq('client_id', client_id)
      .order('created_at', { ascending: false }));
    if (!error && checkpoint !== 't12') return [];   // prima della v62 esistono solo le annuali
  }
  if (!error && data) data = data.filter(r => (r.checkpoint || 't12') === checkpoint);
  if (error) throw error;
...
// Ri-fotografia dei SEI MESI: va a tutta la popolazione che ha fatto il check-up,
// non ai soli pazienti in trattamento (Enrico, 12/9). Aggancio: 180 giorni dalla
// compilazione del check-up iniziale. Chi l'ha già fatta non viene richiamato.
export async function getPatientsForRifotografiaInvite(giorni = 180) {
  const finestraInizio = new Date(Date.now() - (giorni + 7) * 86400000).toISOString();
  const finestraFine = new Date(Date.now() - (giorni - 2) * 86400000).toISOString();
  const { data: pazienti, error } = await supabase
    .from('patients')
    .select('id, first_name, last_name, email, client_id, care_token, consent_withdrawn_at, assessment_completed_at, clients(name)')
    .gte('assessment_completed_at', finestraInizio)
    .lte('assessment_completed_at', finestraFine);
  if (error || !pazienti || !pazienti.length) return [];
  const ids = pazienti.map(p => p.id);
  let fatte = [];
  const r = await supabase.from('reassessments_t12').select('patient_id, checkpoint').in('patient_id', ids);
  if (!r.error) fatte = (r.data || []).filter(x => (x.checkpoint || 't12') === 't6').map(x => x.patient_id);
  const gia = new Set(fatte);
  return pazienti.filter(p => p.email && p.care_token && !p.consent_withdrawn_at && !gia.has(p.id));
}
...
export async function insertGeneratedReport(fields) {
  let { data, error } = await supabase.from('generated_reports')
    ...
    .select().single();
  // Fallback se migration di colonne opzionali non applicate (v29 pdf_url, v36 quote_compliance, v57 ai_status)
  if (error && (fields.pdf_url !== undefined || fields.quote_compliance !== undefined || fields.ai_status !== undefined)) {
    const { pdf_url, quote_compliance, ai_status, ...rest } = fields;
...
export async function updateGeneratedReport(id, fields) {
  const { data, error } = await supabase.from('generated_reports').update(fields).eq('id', id).select().single();
  if (error) throw error;
  return data;
}
...
export async function getBufferStatusByClient(client_id) {
  ...
  // Sedute totali L1 base e buffer: regole del protocollo
  const base_sessions = l1_count * PROTOCOLLO.sedute_per_ciclo;
