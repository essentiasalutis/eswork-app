...
// ─────────────────────────────────────────────────────────────────────────────
import { dataIt, dataOraIt } from './date-it.mjs';
...
// ─── L'anamnesi nella copia dei dati del dipendente (GDPR art. 15, 20) ──────────
// Decisione di Enrico (18/9): originale e integrazioni, con la stessa distinzione
// della cartella: cosa ha dichiarato lui, cosa ha integrato il professionista,
// quando e perché. Il nome dell'osteopata resta: l'interessato ha diritto di sapere
// chi ha scritto cosa sui suoi dati. Le note del professionista sono incluse.
// integrazioni: righe con `osteopata` (il nome) già risolto.
export function anamnesiPerInteressato(doc, integrazioni) {
  if (!doc || doc.type !== 'anamnesi' || !(doc.status === 'completed' || doc.status === 'signed')) return null;
  const originale = originaleDaDocumento(doc);
  const { valori, storia, integrazioni: ordinate } = versioneCorrente(originale, integrazioni);
  const fonteOriginale = campo => (campo === 'pro_notes' ? 'scritto dall\'osteopata nell\'originale' : 'dichiarato da te nell\'originale firmato');
  const voci = (filtro, valoreDi, fonteDi) => SEZIONI_ANAMNESI.flatMap(sez => sez.campi.filter(filtro).map(c => ({
    sezione: sez.titolo, voce: c.etichetta, valore: valoreDi(c), fonte: fonteDi(c),
  })));
  const pieno = (v, c) => valoreLeggibile(c.campo, v) !== '—';
  return {
    come_leggerla: 'La tua firma copre solo ciò che hai dichiarato nell\'originale. Quello che l\'osteopata ha aggiunto o corretto dopo è indicato come integrazione, con la data, il nome del professionista e il motivo; il testo originale resta sempre riportato.',
    firmata_il: dataIt(doc.signed_at),
    modalita_firma: doc.modalita === 'carta' ? 'su carta' : 'in piattaforma',
    originale_firmato: voci(c => pieno(originale[c.campo], c), c => valoreLeggibile(c.campo, originale[c.campo]), c => fonteOriginale(c.campo)),
    versione_corrente: voci(
      c => storia[c.campo] || pieno(valori[c.campo], c),
      c => (storia[c.campo] && svuotato(storia[c.campo].at(-1)) ? 'campo svuotato' : valoreLeggibile(c.campo, valori[c.campo])),
      c => {
        const st = storia[c.campo];
        if (!st) return fonteOriginale(c.campo);
        const u = st[st.length - 1];
        return `${svuotato(u) ? 'rimosso' : 'integrato'} dall'osteopata ${u.osteopata || ''} il ${dataOraIt(u.creato_il)} — motivo: ${u.motivo} — nell'originale: ${valoreLeggibile(c.campo, originale[c.campo])}`.replace('osteopata  il', 'osteopata il');
      },
    ),
    integrazioni_successive_alla_firma: ordinate.map(r => ({
      data: dataOraIt(r.creato_il),
      osteopata: r.osteopata || null,
      voce: CAMPI_ANAMNESI[r.campo]?.etichetta || r.campo,
      prima: valoreLeggibile(r.campo, r.valore_prima),
      dopo: svuotato(r) ? 'campo svuotato' : valoreLeggibile(r.campo, r.valore_dopo),
      motivo: r.motivo,
    })),
  };
}
