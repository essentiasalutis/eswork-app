...
export async function getCheckupInChiusura({ oggi, limite }) {
  try {
    const r = await supabase.from('assessments')
      .select('id, client_id, created_at, chiude_il, clients(name, is_demo)')
    ...
    for (const a of r.data || []) {
      if (await reportAttivazioneDopo(a.client_id, a.created_at).catch(() => true)) continue;
      const c = a.clients || {};
      out.push({ client_id: a.client_id, cliente: c.name || a.client_id, is_demo: !!c.is_demo, chiude_il: a.chiude_il });
    }
    return out;
  } catch (_) { return []; }
}
