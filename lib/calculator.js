...
// ─── Voci di un anno ──────────────────────────────────────────────────────────
function computeYear({ l1, l2, tier, groups, modules, rates, cfg }) {
  ...
  const vat = cfg.vat_exempt ? 0 : total_sell * cfg.vat_pct;
...
export function calculatePricing(arg, l1Arg, l2Arg) {
  ...
  const y1 = computeYear({ l1, l2, tier, groups, modules: cfg.training_modules_y1, rates, cfg });
  const y2 = computeYear({ l1, l2, tier, groups, modules: cfg.training_modules_y2, rates, cfg });
  ...
  return {
    ...
    hours_treated: (cfg.sessions_per_l1 * cfg.session_duration_min) / 60,
