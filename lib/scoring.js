...
export function aggregateNMQ(allAnswers) {
  ...
  if (n === 0) return { zones: [], symptomatic: 0, pct: 0, n };
  ...
  const symptomatic = allAnswers.filter(a => scoreNMQ(a).some(z => z.q12)).length;
  return {
    zones: zones.sort((a, b) => b.pct12 - a.pct12),
    symptomatic,
    pct: Math.round(symptomatic / n * 100),
    n,
  };
}
...
export function trafficLight(type, value) {
  if (type === 'nmq') return value > 50 ? 'red' : value > 30 ? 'yellow' : 'green';
  if (type === 'pss') return value > 20 ? 'red' : value >= 14 ? 'yellow' : 'green';
  if (type === 'uwes') return value < 2.5 ? 'red' : value < 4.5 ? 'yellow' : 'green';
  if (type === 'enps') return value < 0 ? 'red' : value < 20 ? 'yellow' : 'green';
  return 'gray';
}

export function generateSummaryText(nmq, pss, uwes, enps) {
  const lines = [];
  const nmqColor = trafficLight('nmq', nmq.pct);
  if (nmqColor === 'red') {
    lines.push(`Il ${nmq.pct}% dei dipendenti ha riportato disturbi muscolo-scheletrici negli ultimi 12 mesi — dato critico che richiede intervento prioritario.`);
  } else if (nmqColor === 'yellow') {
    lines.push(`Il ${nmq.pct}% dei dipendenti presenta disturbi muscolo-scheletrici: area da monitorare. La zona più colpita è ${nmq.zones[0]?.zone || 'non rilevata'} (${nmq.zones[0]?.pct12 || 0}%).`);
  } else {
    lines.push(`Il ${nmq.pct}% dei dipendenti ha riportato disturbi muscolo-scheletrici, con la zona ${nmq.zones[0]?.zone || ''} come area prioritaria.`);
  }
  if (pss) {
  ...
  else if (uwesColor === 'yellow') lines.push(`L'engagement è nella media (UWES-9: ${uwes.mean}/6): margini di miglioramento su ${uwes.vigore < uwes.dedizione ? 'vigore' : 'dedizione'}.`);
