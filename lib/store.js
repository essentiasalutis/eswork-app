...
export async function insertGeneratedReport(fields) {
  let { data, error } = await supabase.from('generated_reports')
    ...
    .select().single();
  // Fallback se migration di colonne opzionali non applicate (v29 pdf_url, v36 quote_compliance)
  if (error && (fields.pdf_url !== undefined || fields.quote_compliance !== undefined)) {
    const { pdf_url, quote_compliance, ...rest } = fields;
