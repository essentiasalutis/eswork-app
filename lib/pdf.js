...
export function buildQuoteHtml({ client, forchetta, sector_label, v2Doc }) {
  ...
  // Sezione componenti (SOLO listino v2): elenco senza tariffe; l'ergonomia
  // produzione compare come voce STIMATA "da confermare con sopralluogo".
  const v2Section = v2Doc ? `
    <h2>Cosa comprende il programma</h2>
    <div class="why">
      <p>• <strong>Percorsi clinici individuali</strong> per chi presenta un bisogno effettivo (valutazione e cicli di trattamento in sede).</p>
      <p>• <strong>Prevenzione attiva</strong> per chi presenta segnali iniziali (sessioni individuali dedicate).</p>
      <p>• <strong>Formazione collettiva</strong> su postura e prevenzione (gruppi in sede).</p>
      <p>• <strong>Consulenza ergonomico-posturale</strong> sulle postazioni di lavoro${v2Doc.ergonomiaAddetti > 0 ? ', con la formazione degli addetti di reparto alla postura corretta sulla propria postazione' : ''}${v2Doc.ergonomiaPostazioni > 0 ? ' — <em>la parte relativa alle postazioni di produzione è stimata e verrà confermata con il sopralluogo</em>' : ''}.</p>
    </div>` : '';
