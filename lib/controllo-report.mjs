...
const VIETATI = [
  ...
  [/\bsanitari[oae]?\b/i, '«sanitario» riferito al servizio'],
  ...
  [/in linea con/i, 'paragone («in linea con») non fornito'],
  ...
  [/(?<![\d.,])(19|20)\d\d(?![\d.,])/, 'anno citato ma non fornito'],
...
export function numeriDi(testo) {
  return (String(testo || '').match(RE_NUM) || []).map(valore).filter(n => Number.isFinite(n));
}
