...
// (il quanto). Il blocco "Standard e sostenibilità" è OPZIONALE: lo si accende con
// una spunta sulla pagina Presenta, caso per caso (default spento).
...
// Blocco 2 — Leve economiche (quanto vale). Mai euro accanto alle voci del programma: la
// sola cifra qui è lo scenario delle assenze. L'OT23 non ha cifre né percentuali finché
// l'INAIL non risponde sull'ammissibilità del programma (Enrico, 21/9: lib/ot23-stato.mjs).
export function leveEconomiche({ giorniMalattia, giorniMsk, incidenzaPct = 10, costoGiornata = 160 } = {}) {
  const out = [];
  out.push({
    titolo: 'Domanda OT23 all\'INAIL',
    testo: TESTO_OT23_IN_VERIFICA,
    nota: 'La domanda la presenta l\'azienda, direttamente o tramite il consulente del lavoro.',
  });
  out.push({
    titolo: 'Deducibilità',
  ...
  out.push({ titolo: 'Welfare aziendale', testo: 'Il programma si integra con i piani welfare aziendali e con gli strumenti di flexible benefit già attivi.' });
  out.push({ titolo: 'Tempo richiesto', testo: `${fraseTempoRichiesto({ maiuscola: true })}: impatto minimo sull'operatività.` });
  return out;
}

// Blocco opzionale: serve alle aziende che redigono la rendicontazione di sostenibilità.
// A un titolare di micro-impresa non dice nulla — per questo si sceglie al momento.
...
export function noteLevePerEnrico({ dipendenti } = {}) {
  return [
    ...
    `OT23: l'ammissibilità del programma come intervento C-4.1 è in verifica con l'INAIL. Fino alla risposta scritta non citare percentuali né cifre di riduzione, nemmeno a voce. Solo per te: la fascia stimata sui dipendenti sarebbe ${fasciaOT23(dipendenti)}% (INAIL la calcola sui lavoratori-anno della posizione assicurativa; 8% fisso nei primi due anni).`,
  ];
}
