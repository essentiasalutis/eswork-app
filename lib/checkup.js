...
export function etichettaData(ymd) {
  if (!isYmd(ymd)) return '';
  const [y, m, d] = ymd.split('-').map(Number);
  return new Date(Date.UTC(y, m - 1, d, 12)).toLocaleDateString('it-IT', { day: 'numeric', month: 'long', timeZone: 'UTC' });
}
...
un aggiornamento sul check-up: finora sono arrivati ${n} questionari${su}${pct}. Il link resta aperto fino al ${etichettaData(chiudeIl)}. Chi non risponde entro quella data non rientra nell'analisi: un ultimo promemoria ai colleghi fa spesso la differenza.
