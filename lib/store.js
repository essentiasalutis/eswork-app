...
export async function deleteClientById(id) {
  // Cancella sessioni dei pazienti di questo cliente
  await supabase.from('sessions').delete().in(
    'patient_id',
    supabase.from('patients').select('id').eq('client_id', id)
  );
  // Cancella pazienti
  await supabase.from('patients').delete().eq('client_id', id);
  // Cancella assegnazioni professionisti
  await supabase.from('professional_assignments').delete().eq('client_id', id);
  // Cancella risposte assessment
