...
import { conProtocollo, eRegolaDelProtocollo } from '../protocollo.mjs';
import { vociListinoConSospensione, rifiutoScritturaOT23 } from '../ot23-stato.mjs';

const NUMERIC_KEYS = new Set(Object.keys(DEFAULTS_V2));

// Nota in fondo a OGNI report (Attivazione + checkpoint), a schermo e nel PDF.
// DUE varianti scelte da ai_status: il report generato dall'AI lo dichiara, quello
// scritto dalla piattaforma no. Nessuna delle due dichiara una validazione
// professionale: è vera nei fatti ma non registrata da nulla, e su un documento
// sanitario non si stampa un'assunzione di responsabilità che non si può dimostrare
// (Enrico, 12/9). Tornerà, con la spunta "validato da [nome], [data]".
// Editabili da admin (pricing_settings v2, chiavi 'nota_report' e 'nota_report_ai');
// default nel codice come rete di sicurezza (mai un report senza nota).
export const DEFAULT_NOTA_REPORT = 'Questo report è stato elaborato sui dati aggregati della vostra azienda secondo il protocollo ES Work.';
export const DEFAULT_NOTA_REPORT_AI = 'Questo report è stato elaborato sui dati aggregati della vostra azienda secondo il protocollo ES Work, con il supporto di strumenti di intelligenza artificiale.';

export async function getNoteReport() {
  try {
    const { data } = await supabase.from('pricing_settings').select('key,value').eq('version', 'v2').in('key', ['nota_report', 'nota_report_ai']);
    const m = Object.fromEntries((data || []).map(r => [r.key, r.value]));
    return { base: m.nota_report || DEFAULT_NOTA_REPORT, ai: m.nota_report_ai || DEFAULT_NOTA_REPORT_AI };
  } catch { return { base: DEFAULT_NOTA_REPORT, ai: DEFAULT_NOTA_REPORT_AI }; }
}
...
export async function getPricingSettingsV2() {
  ...
  try {
    ...
    for (const row of data || []) {
      if (eRegolaDelProtocollo(row.key)) continue;   // regola del protocollo: non viene dal DB
  ...
  return { params: conProtocollo(params), texts };
}

export async function updatePricingSettingV2(key, value) {
  if (eRegolaDelProtocollo(key)) throw new Error(`«${key}» è una regola del protocollo: non si modifica dal Listino (lib/protocollo.mjs).`);
...
export async function getServiziDeliverable({ soloAttivi = false, configurazione = null } = {}) {
  ...
  if (error) throw new Error(error.message);
  return vociListinoConSospensione(data || []);   // voce OT23 sospesa: vale 0 (lib/ot23-stato.mjs)
}

export async function updateServizioDeliverable(id, fields) {
  const rifiuto = rifiutoScritturaOT23(id, fields);
  if (rifiuto) { const e = new Error(rifiuto); e.status = 422; throw e; }
